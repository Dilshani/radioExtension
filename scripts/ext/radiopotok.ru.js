!function () {
    ...
    function d(a) {
        c();
        var d = document.getElementsByTagName("head")[0], e = a.parentNode.parentNode, f = [], g = e.querySelector("a[rel=canonical]"), h = d.querySelector('meta[property="og:url"]'), i = g && g.href || h && h.content, j = d.querySelector('meta[property="og:description"]'), k = a.dataset.title || g && g.textContent || j && j.content, l = e.parentNode.parentNode.querySelector(".img-thumbnail"), m = d.querySelector('meta[property="og:image"]'), n = l && l.src.replace("/s_", "/") || m && m.content.replace("/s_", "/");
    ...
    var e = document.querySelectorAll(".btn.play-radio");
    if (e.length) {
        a();
        for (var f = 0, g = e.length; g > f; f++) {
            var h = document.createElement("button");
            h.type = "button", h.className = "online-radio-add-button btn btn-default", h.title = chrome.i18n.getMessage("add"), h.onclick = d.bind(h, e[f]), e[f].parentNode.insertBefore(h, e[f])
        }
    }
}();